...
//  Testing: Modules/Core/Tests/CoreTests/BudgetServiceTests.swift
//
//  Description: Service for budget management and monitoring providing analytics,
//  spending tracking, category budgeting, and alert management. Integrates with
//  repository layer for budget persistence and item cost tracking.
//
...
public final class BudgetService {
    ...
    private func createExceededAlert(for budget: Budget, status: BudgetStatus) async throws {
        let alert = BudgetAlert(
            ...
            message: "You've exceeded your \(budget.name) budget by $\(String(format: "%.2f", NSDecimalNumber(decimal: status.spent - budget.amount).doubleValue))",
            percentageUsed: status.percentageUsed,
            amountSpent: status.spent,
            amountRemaining: status.remaining
        )
    ...
    private func createProjectedAlert(for budget: Budget, status: BudgetStatus, projected: Decimal) async throws {
        let alert = BudgetAlert(
            ...
            message: "At current spending rate, you'll exceed your \(budget.name) budget by $\(String(format: "%.2f", NSDecimalNumber(decimal: projected - budget.amount).doubleValue))",
            percentageUsed: status.percentageUsed,
            amountSpent: status.spent,
            amountRemaining: status.remaining
        )
